    ...
    **/
    ...
    hook : function(target, event, func, args) {
        ...
        if (target instanceof Tea.Object) {
            ...
            target.on(event, null, args, handler);
        }
...
Tea.Scrim = Tea.Element.extend('t-scrim', {
    ...
    show : function()
    {
        ...
        source.hide().fadeTo('fast', .6);
        source.click(Tea.method(this.hide, this));
    },
    hide : function()
    {
        this.remove();
...
Tea.CheckBoxInput.Skin = Tea.Skin.extend('t-checkbox-input-skin', {
    getValue : function()
    { 
        return this.source.attr('checked') == 'checked';
    },
    setValue : function(v)
    {
        if (v)
            this.source.attr('checked', 'checked');
        else
            this.source.removeAttr('checked');
    },
    setFocus : function(flag)
    {
        if (flag)
...
Tea.Button.Skin = Tea.Skin.extend('t-button-skin', {
    ...
    render : function(source) {
        ...
        source.bind('click touch', Tea.method(element.performClick, element));
...
Tea.Popup = Tea.Container.extend('t-popup', {
    ...
    show : function() {
        ...
        this.hook($(document), 'click touch', Tea.latent(100, this.hide, this));
